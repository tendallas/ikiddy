...
class CBreadcrumbs extends CWidget
{
	 ...
	 */
	public $htmlOptions=array('class'=>'breadcrumbsmicro');
	/**
	...
	public function run()
	{
		...
		if($this->homeLink===null)
			$links[]=CHtml::openTag('div', array('itemscope'=>'', 'itemtype' => 'http://data-vocabulary.org/Breadcrumb')).
					CHtml::openTag('a', array('itemprop' => 'url', 'href' => Yii::app()->homeUrl)).
					CHtml::openTag('span', array('itemprop' => 'title')).
					Yii::t('zii','Home').
					CHtml::closeTag('span').
					CHtml::closeTag('a').
					CHtml::closeTag('div');
		...
		foreach($this->links as $label=>$url)
		{
			if(is_string($label) || is_array($url))
				$links[]=CHtml::openTag('div', array('itemscope'=>'', 'itemtype' => 'http://data-vocabulary.org/Breadcrumb')).
					CHtml::openTag('a', array('itemprop' => 'url', 'href' => $url)).
					CHtml::openTag('span', array('itemprop' => 'title')).
				...
				$links[]=CHtml::openTag('div', array('itemscope'=>'', 'itemtype' => 'http://data-vocabulary.org/Breadcrumb')).'<span itemprop="title">'.($this->encodeLabel ? CHtml::encode($url) : $url).'</span>'.CHtml::closeTag('div');
		}
