...
if(!empty($manufacturers['filters']) || !empty($attributes))
	...
	// Menu
	echo CHtml::openTag('div', array('class'=>'filter_header'));
	echo CHtml::encode(Yii::t('StoreModule.core', 'Категории'));
	echo CHtml::closeTag('div');
	...
	if(!empty($manufacturers['filters']))
	{
		...
		foreach($manufacturers['filters'] as $filter)
		{
			...
			if(in_array($filter['queryParam'], $queryData))
			{
				...
				echo CHtml::openTag('input', array('type' => 'checkbox', 'value' => $filter['count'], 'checked' => 'checked', 'onclick' => 'window.location.href="'.$url.'"'));
				echo CHtml::link($filter['title'], $url, array('style'=>'color:green'));
			}
			elseif($filter['count'] > 0) {
				echo CHtml::openTag('input', array('type' => 'checkbox', 'value' => $filter['count'], 'onclick' => 'window.location.href="'.$url.'"'));
				echo CHtml::link($filter['title'], $url).' <sup>'.$filter['count'].'</sup>';
			} else {
				echo CHtml::openTag('input', array('type' => 'checkbox', 'value' => $filter['count'], 'disabled' => 'disabled'));
				echo $filter['title'].' <sup>0</sup>';
			}
	...
	{
		if(CHtml::encode($attrData['title']) == 'Цвет'){
			echo CHtml::openTag('div', array('class'=>'filter_header'));
			echo CHtml::encode($attrData['title']);
			echo CHtml::closeTag('div');
			echo CHtml::openTag('div', array('class' => 'filter_links colours'));
			...
			foreach($attrData['filters'] as $filter)
			{
				...
				$color = Yii::app()->db->createCommand()
				    ->select('code')
				    ->from('FilterColours')
				    ->where('rus_name=:name', array(':name'=> $filter['title']))
				    ->queryRow();
				echo CHtml::openTag('div', array('class' => 'colour', 'style' => 'background: ' . $color['code']));
				// Filter link was selected.
				if(in_array($filter['queryParam'], $queryData)) {
					$url = Yii::app()->request->removeUrlParam('/store/category/view', $filter['queryKey'], $filter['queryParam']);
					echo CHtml::openTag('input', array('type' => 'checkbox', 'value' => $color['code'], 'checked' => 'checked', 'onclick' => 'window.location.href="'.$url.'"'));
				} elseif ($filter['count'] > 0) {
					echo CHtml::openTag('input', array('type' => 'checkbox', 'value' => $color['code'], 'onclick' => 'window.location.href="'.$url.'"'));
				} /*else {
					echo CHtml::openTag('input', array('type' => 'checkbox', 'value' => $color['code'], 'disabled' => 'disabled', 'onclick' => 'window.location.href="'.$url.'"'));
				}*/
		...
		} elseif(CHtml::encode($attrData['title']) == 'Размер (Возраст)'){
			echo CHtml::openTag('div', array('class'=>'filter_header'));
			echo CHtml::encode($attrData['title']);
			echo CHtml::closeTag('div');
			echo CHtml::openTag('div', array('class' => 'filter_links sizeage'));
