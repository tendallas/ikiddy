<table class="configurations rc5">
	...
	foreach($model->processVariants() as $variant)
	{
		...
		foreach($variant['options'] as $v)
		{
			...
			if (empty($v->option->value)) {
				$dropValue = Yii::app()->db->createCommand()
				    ->select('trans.value AS name')
				    ->from('StoreAttributeOptionTranslate trans, StoreProductVariant var')
				    ->where('var.id=:name AND var.option_id = trans.object_id AND trans.language_id = 1', array(':name'=> $v->id))
				    ->queryRow();
				$dropDownData[$v->id] = $dropValue['name'];
			} else {
